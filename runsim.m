...
for t = 1:numSteps
    ...
    for i = 1:State.Ekf.nL
        plotcov2d(State.Ekf.mu(2+2*i),State.Ekf.mu(3+2*i), State.Ekf.Sigma(2+2*i:3+2*i,2+2*i:3+2*i),'red', 0, 'red', 0, 3);
    end
    drawnow;
    if pauseLen > 0
        pause(pauseLen);
...
% Plot features association
figure(3)
observation = Data.realObservation(:,:,t);
plotfield(observation(3,:));hold on
legendDA = cell(1,State.Ekf.nL);
p = [];
for i = 1:State.Ekf.nL
    landmarkX = State.Ekf.mu(2+2*i);landmarkY = State.Ekf.mu(3+2*i);
    p = [p plot(landmarkX,landmarkY,'p','MarkerSize', 15,'LineWidth',2)];
    hold on;
    legendDA{i} = ['Observation',num2str(State.Ekf.sL(i))];
end
legend(p,legendDA);
title('The data association result')
if nargout >= 1
    varargout{1} = Data;
end
%--------------------------------------------
