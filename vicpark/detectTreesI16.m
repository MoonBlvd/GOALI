function x=detectTreesI16(RR,NofI) 
...
daMin2 = 2*pi/360 ;
% --------------------------------
global AAr; AA = AAr ;% angle in radius,0-2pi radius
ii1=Find16X(RR<M11) ; % find close objects
L1 = length(ii1); % find # of close objects
if L1<1, x=[] ; return ; end ;
R1 = RR(ii1) ; A1 = AA(ii1) ; % get all close objects R1,

ii2 = find(  (abs(diff(R1))>M2)|( diff(A1)>M2a) ) ; % find object which is not too close to the next object 
...
x2  =  R2.*cos(A2 ) ; y2 =  R2.*sin(A2 ) ;% x, y coordinates from rob to objects
